...
(function (window, document) {
    ...
    Croppic.prototype = {
        ...
        bindImgUploadControl: function () {
            ...
            var formHtml = '<form class="' + that.id + '_imgUploadForm" style="display: none; visibility: hidden;"></form>';
            var file = $('<input/>').attr({name:that.options.postName,type:"file"});
            that.form = $(formHtml).append(file);
            that.outputDiv.append(that.form);
            //that.form = that.outputDiv.find('.' + that.id + '_imgUploadForm');

            that.imgUploadControl.off('click');
            that.imgUploadControl.click(function () {
                that.img = $('<img class="preview"/>').css("max-width", "none");
                file.click();
            });
        ...
        preview: function (file) {
            ...
            if (file.files && file.files[0]) {
                ...
                reader.onload = function (evt) {
                    that.img.attr('src', evt.target.result);
                };
                reader.readAsDataURL(file.files[0]);
            } else {
                file.select();
                var src = document.selection.createRange().text;
                that.img[0].filters.item('DXImageTransform.Microsoft.AlphaImageLoader').src = src;
            }
        ...
        upload: function () {
            ...
            that.uploadbtn.attr('disabled', true);
            $.ajax({
                ...
                xhr: function () {
                    ...
                    if(typeof that.options.onUploadProgress == 'function'){
                        xhr.upload.addEventListener("progress", that.options.uploadProgress, false);
                    }

                    //Download progress
                    if(typeof that.options.onDownloadProgress == 'function'){
                        xhr.addEventListener("progress", that.options.onDownloadProgress, false);
                    }
...
//                            that.createModal();
                    ...
                    that.imgUrl = response.src;
                    that.img.attr('src', response.src);
//                        that.obj.append('<img src="' + response.src + '">');
//                        that.initCropper();
//
                    that.hideLoader();
                ...
                if (response.status ==0) {
                    that.obj.append('<p style="width:100%; height:100%; text-align:center; line-height:' + that.objH + 'px;">' + response.info + '</p>');
                    that.hideLoader();
                    setTimeout(function () {
                        that.reset();
                    }, 2000);
        ...
        createModal: function () {
            ...
            var btn_panel = $('<div style="z-index: 2;position: absolute;height: auto;display: block;bottom: -31px;right:0;font-family: sans-serif;background-color: rgba(0,0,0,0.35);"></div>');
            that.uploadbtn = $('<button type="button" class="btn btn-success" disabled="disabled">Upload</button>').click(function (e) {
                that.crop();
            });
            btn_panel.append(that.uploadbtn);
            that.obj.append(btn_panel);
        },
        ...
        initialScaleImg: function () {
            var that = this;
            that.imgW = that.img.width();
            that.imgH = that.img.height();
        ...
        createCropControls: function () {
            ...
            var cropControls = {
                zoomMuchIn: '<i class="cropControlZoomMuchIn"></i>',
                zoomIn: '<i class="cropControlZoomIn"></i>',
                zoomOut: '<i class="cropControlZoomOut"></i>',
                zoomMuchOut: '<i class="cropControlZoomMuchOut"></i>',
                crop: '<i class="cropControlCrop"></i>',
                reset: '<i class="cropControlReset"></i>'
            };
            $.each(that.options.hideControls, function (i, v) {
                if (cropControls[v]) {
                    delete cropControls[v];
                }
            });

            if (!that.options.doubleZoomControls) {
                delete cropControls['zoomMuchIn'];
                delete cropControls['zoomMuchOut'];
            }
            // CREATE CONTROLS
            that.cropControlsCrop = $('<div class="cropControls cropControlsCrop"></div>');
            $.each(cropControls, function (k, v) {
                that.cropControlsCrop.append(cropControls[k]);
            });

            that.obj.append(that.cropControlsCrop);
        ...
        zoom: function (x) {
            ...
            if ((newWidth - that.objW) <= 0 && (newHeight - that.objH) <= 0) {
            ...
            var maxW = 2 * that.objW;
            var maxH = 2 * that.objH;
            ...
            if (that.options.onImgZoom){
                that.uploadbtn.attr('disabled', false);
        ...
        crop: function () {
            ...
            $.ajax({
                ...
                xhr: function () {
                    ...
                    //Upload progress
                    if(typeof that.options.onUploadProgress == 'function'){
                        xhr.upload.addEventListener("progress", that.options.onUploadProgress, false);
                    }
                ...
                that.uploadbtn.removeAttr('disabled');
                ...
                if (response.status ==1) {
                    ...
                    if(response.src){
                        that.obj.append('<img class="croppedImg" src="' + response.src + '">');
                        if (that.options.outputUrlId !== '') {
                            $('#' + that.options.outputUrlId).val(response.src);
                        }
                    }
                ...
                if (!response.status) {
                    that.obj.append('<p style="width:100%; height:100%;>' + response.info + '</p>">');
                }
        ...
        reset: function () {
            var that = this;
            that.destroy();
            that.uploadbtn&&that.uploadbtn.removeAttr('disabled');
            that.init();
            if (!$.isEmptyObject(that.croppedImg)) {
